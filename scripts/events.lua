function on_first_tick()
    ...
    main_surface = game.surfaces.nauvis
end
...
function event_on_train_changed_state(e)
    ...
    if e.train.state == defines.train_state.wait_station then
        ...
        if (is_dispatcher(e.train.station.backer_name)) then
            make_train_wait_at_dispatcher(e.train)
            local unitName = get_unit_name(e.train.station.backer_name)
...
function is_endpoint_of_unit(endpointStopName, unitName)
    if unitName == nil or not units[unitName] then
        return false
    end
...
function get_unit_name(stopName)
    if is_dispatcher(stopName) then
        return stopName:sub(1, stopName:len() - get_dispatcher_postfix():len())
    else
        for unitName, _ in pairs(units) do
            if is_endpoint_of_unit(stopName, unitName) then
                return unitName
            end
        end
    end
...
function refresh_units()
    local stops = main_surface.find_entities_filtered{type = "train-stop" }
    table.clear(units)

    -- find unit names
    for _, stop in ipairs(stops) do
        if is_dispatcher(stop.backer_name) then
            units[get_unit_name(stop.backer_name)] = { unitName = stop.backer_name, endpoints = {} }
        end
    end
        ...
        if not is_dispatcher(stop.backer_name) then
            for unitName, unitData in pairs(units) do
                if string.starts(stop.backer_name, unitName) then
                    unitData.endpoints[stop.backer_name] = stop
                end
            end
        end
    end
...
function make_train_wait_at_dispatcher(train)
    ...
    elseif not is_endpoint_of_unit(new_schedule.records[nextRecIdx].station, get_unit_name(dispatcherName)) then
